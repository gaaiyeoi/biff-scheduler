...
export function buildGuideBody(cat: Catalog): HTMLElement {
  ...
  body.appendChild(guideH("网格与行程图例"));
  {
    ...
    const lines: [string | HTMLElement, string][] = [
      ["绿底 · 已选", "已加入当前方案(A/B)的场次 — 整卡淡绿底。优先级不染网格卡,请在下方行程行三段 seg 设置(蓝/紫/灰蓝 = 必看/备选/随缘),冲突取舍与抢票顺位按此排"],
  ...
  // ---- 7 我的选片(打标)----
  body.appendChild(guideH("我的选片(打标)"));
  {
    const ul = el("ul", "grid gap-[3px]");
    [
      ["必看 / 备选 / 随缘", "在「影片库」片名行右侧、或影片详情弹层里给影片打标(再点同档取消)。打标只记「想看什么」,不直接改排片"],
      ["甘特色点", "打标后,甘特卡标题行前出现 7px 圆点(蓝=必看 / 紫=备选 / 灰蓝=随缘)——与整卡红绿灯底色相互独立:底色说「排得怎么样」,色点说「是不是我想看的」。档位刻意用冷色系(蓝/紫/灰蓝),避开底色的红/黄/绿,保证落在任何底色卡上都一眼可辨"],
      ["顶栏「我的选片」", "汇总全部已打标影片(带计数),可按档位筛选、看场次摘要、跳转定位、一键取消打标"],
      ["智能排片", "按打标本地求解建议行程:同一天不重叠 + 跨馆缓冲;必看尽量全覆盖(冲突给牺牲说明),备选按评分/GV 填空,随缘不自动排"],
    ].forEach(([k, v]) => ul.appendChild(el("li", "text-[12.5px] leading-[1.6]", `${k} — ${v}`)));
    body.appendChild(ul);
    body.appendChild(
      note(
        "注意区分两层「档位」:行程行(我的行程)里的三段 seg 是「已排场次的优先级」,作用于已选场次(参与质量分与抢票顺位);影片库打标作用于影片本身(选片意向)。两者同色系、互不影响。新加入行程的场次按该片打标自动继承档位;影片库未打标的场次为「未设」(三段全灰、左侧标「未设」),不再默认成「备选」——未设不参与质量分。"
      )
    );
  }
