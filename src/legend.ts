...
export function buildGuideBody(cat: Catalog): HTMLElement {
  ...
  body.appendChild(guideH("我的选片"));
  {
    ...
    [
      ...
      ["顶栏「影片库 · 选片」", "一个按钮 = 一个弹窗的两栏:左栏 = 全部影片(搜索 / 单元筛选),右栏 = 我的选片(两排 chips 筛选:**日期** / **档位**;看已排场次、逐场定位、整片移除)。两栏共用同一套影片行,所以打标 / 图标 / 场次行完全一致;右栏展开只列**已排场次**,并**按日期分节**(节头给「10/21 周三 · N 场」,行内只留时间)"],
