...
function buildRow(
  ...
  // 时间列(<b>继承 13px 加粗;.a-dur span 另设)
  const timeCol = el("div", "flex flex-col text-[13px] tabular-nums");
  timeCol.innerHTML = `<b>${escapeHtml(s.start_time)}–${escapeHtml(s.end_time)}</b><span class="block text-[11px] text-muted">${s.duration_min}min</span>`;
  if (prev && !entryConf) {
    timeCol.appendChild(gapNote(prev, s, ctx.transitMin));
  }
...
function gapNote(prev: Screening, s: Screening, transitMin: number): HTMLElement {
  const gap = hmsToMin(s.start_time) - hmsToMin(prev.end_time);
  const cross = prev.venue_id !== s.venue_id;
  const need = cross ? transitMin : 0;
  const slack = gap - need;
  const baseCls = "block text-[10.5px] text-muted mt-[2px]";
  let state = "ok";
  let txt = `↖ 距上一场 ${gap}min${cross ? " · 跨馆" : ""}`;
  let stateCls = "";
  if (slack < 0) {
    state = "bad";
  ...
  g.title = cross
    ? `跨馆转场:间隔 ${gap}min − 缓冲 ${need}min = 余量 ${slack}min(${state === "bad" ? "不足" : state === "tight" ? "偏紧" : "宽裕"})`
    : `同馆相邻:间隔 ${gap}min(余量 ${gap}min)`;
  return g;
}
