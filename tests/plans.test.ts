...
describe("buildPlanSet:无冲突", () => {
  it("全部场次都进 common,只有 1 套方案", () => {
    ...
    const ps = buildPlanSet(["001", "002"], conflictsOf(slots), new Map(), byCode);
...
describe("buildPlanSet:单冲突组", () => {
  ...
  it("显式顺位覆盖兜底序(拖动的效果)", () => {
    ...
    const ps = buildPlanSet(["001", "002"], conflictsOf(slots), rank, byCode);
...
describe("buildPlanSet:链式重叠(连通分量)", () => {
  ...
  it("一组三场 → 3 套方案,成本 1/2/3", () => {
    const ps = buildPlanSet(["001", "002", "003"], conflictsOf(slots), new Map(), byCode);
...
describe("buildPlanSet:多组 → 枚举所有组合", () => {
  ...
  it("2 组 × 2 场 → 4 套,按成本 2 / 3 / 3 / 4 排序", () => {
    const ps = buildPlanSet(codes, conflictsOf(slots), new Map(), byCode);
    expect(ps.groups).toEqual([
      ["005", "027"],
      ["071", "089"],
    ]);
  ...
  it("共同场次进每一套,且不进 picks", () => {
    ...
    const ps = buildPlanSet([...codes, "900"], conflictsOf(withCommon), new Map(), byCode);
  ...
  it("换顺位 → 方案顺序整体改变(但集合不变)", () => {
    ...
    const ps = buildPlanSet(codes, conflictsOf(slots), rank, byCode);
  ...
  it("3 组 → 8 套(组合数 = ∏ 组大小)", () => {
    ...
    const ps = buildPlanSet(c, conflictsOf(s), new Map(), () => 0);
...
describe("buildPlanSet:上限与入参", () => {
  it("组合数超过上限 → 只保留成本最低的一批并置 truncated", () => {
    ...
    const ps = buildPlanSet(codes, conflictsOf(slots), new Map(), () => 0);
  ...
  it("重复 code 去重保序;不在 cat 里的 code 照常参与(不抛)", () => {
    ...
    const ps = buildPlanSet(["001", "001", "zzz"], conflictsOf(slots), new Map(), byCode);
    expect(ps.common).toEqual(["001", "zzz"]);
    expect(ps.options[0].codes).toEqual(["001", "zzz"]);
  });

  it("空输入 → 1 套空方案", () => {
    const ps = buildPlanSet([], new Map(), new Map(), byCode);
