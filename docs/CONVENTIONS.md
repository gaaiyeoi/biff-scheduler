...
- **影片卡片信息层级**(2026-09-10 重排,`PLAN-20260910184745` §8;`library.ts::filmRow` / `showRow`):
  ...
  - **★ 事实信息 = 纯文本,描边章只留给「这场不一样」(2026-09-11 五改,对齐 BIFF 官方册子影片页)**:
    册子的元信息行是 `Korea | 2025 | 86min | DCP | color/b&w` —— **一个框都没有**;
    框只出现在右上角那枚 `WP`(World Premiere)。我们此前把片长 / 等级 / 字幕 / 页码全做成描边章,
    五枚 ≈190px,把抽屉 520px 的第 1 行吃光 → 折行。
    现在(`legend.ts::appendMetaRow` 的 `factsAsText` 分支 = **唯一实现点**):
    · **事实**(片长 / 等级 / 字幕 / 页码)= 纯文本 + `·` 分隔(≈120px);**等级保留强调色**
      (「未满岁不得入场」是硬性准入信息,去框但不去色 —— 见 `RATING_TEXT` / `RATING_ACCENT` 并存);
    · **特殊节目**(GV / 首映 / 大师班…)= 徽章 —— 徽章是**稀缺资源**,只标「这场不一样」;
    · 日期只印 `9/21`(册子同款),**星期进 tooltip**。
    ⚠ **只有抽屉里的场次行传 `factsAsText`**;网格卡 / 行程卡 / 弹层仍走默认描边章流 ——
      那些地方空间够,且实心章是「一眼看到有映后谈」的主信号。别顺手改全局。
