...
- **影片卡片信息层级**(2026-09-10 重排,`PLAN-20260910184745` §8;`library.ts::filmRow` / `showRow`):
  ...
  - **★ 场次行 = 两层栅格(2026-09-11 三改,`row.ts::SHOW_ROW_CLS`)**:
    旧版 `flex flex-wrap` + 操作组 `ml-auto` —— 空间不够时操作组被挤到第 2 行**并右对齐**,
    留下「[章组] …… [定位 ▸][＋加入]」这种右侧孤立行(用户截图否掉:「这样的暴力换行很丑」)。
    现在折行位置是**设计好的**:宽(容器 ≥ 540px)一行 `[身份][章组][操作]`;
    窄则身份 + 操作留在第 1 行、**章组整层下沉并左对齐**。
    ⚠ `WHEN_CLS` / `WHERE_CLS` / `ACTS_CLS` / `EXTRA_CLS` 四个定位类是**配套的**,改一个要一起看;
      `WHEN_CLS` **刻意不给 `min-w-0`**(内容是 `whitespace-nowrap`,允许收缩只会挤到溢出)。
    ⚠ 容器查询锚点 = 抽屉 `panel` 上的 `@container`(`library.ts::openFilmPicker`)——
      一处标记覆盖三个 tab(影片库 / 我的选片的列表自己也有 `@container`,就近生效,宽度只差 4px)。
      **别把它挪走**,否则场次行永远停在「窄」那一档。
  ...
  - **`PICKER_W_MIN = 400`(2026-09-11 三改:360 → 400)是「排版阈值」,不是随手取整**:
    场次行第 1 层 = 身份(≈200px)+ 操作组(≈120px)+ 间距 ≈ 330px,加行内距 24 + 抽屉内距 24 = 378 → 取 400。
    再窄第 1 层就放不下、会被抽屉裁掉 —— 所以**到这里就到底**;继续往左拖 = **松手收起抽屉**
    (`PICKER_W_DRAG_FLOOR` 150 只是视觉地板,用来给「我要收起它」一个动作回执)。
